...
static void draw2(int dri_fd, struct modeset_buf *buf, uint8_t r, uint8_t g, uint8_t b, unsigned int x, unsigned int y)
{
...
void refreshScreenCircle(int dri_fd,uint8_t r, uint8_t g,uint8_t b)
{
    ...
    //for (iter = modeset_list; iter!=NULL; iter = iter->next) {
        ...
        //        draw2(dri_fd,buf,0x00,0x00,0x00,x,y);

        draw_circle(dri_fd,buf,point.x,point.y,40,r,g,b);

        iter->saved_crtc->fb_id = buf->fb;
        ret = ioctl(dri_fd, DRM_IOCTL_MODE_SETCRTC, iter->saved_crtc);
        if (ret){
...
void refreshScreenColor(int dri_fd,uint8_t r, uint8_t g,uint8_t b, bool r_up, bool g_up, bool b_up)
{
...
int modeset_create_fb(int dri_fd, struct modeset_buf *buf){
    //------------------------------------------------------------------------------
...
int main()
{
    ...
    int x = modeset_list->bufs->width/2;
    int y = modeset_list->bufs->height/2;
    ...
    while(1)
    {
        ...
        point.x = x;
        point.y = y;
        refreshScreenCircle(dri_fd,r,g,b);
        //usleep(500000);
    };
